...
type BookUpdater interface {
	UpdateBook(tx *sql.Tx, book Book) error
...
func (b *BookUpdaterImpl) UpdateBook(tx *sql.Tx, book Book) error {
	// Invalidate caches
...
func (b *BookUpdaterImpl) UpdateFormats(ctx context.Context, bookID int, newFormats []string) (err error) {
	// Invalidate cache for bookID
	formatsCache.Delete(bookID)
	b.Logger.Info("Invalidating formats cache for book", "bookID", bookID)

	// Fetch current formats for the book
	...
	defer func() {
		if err != nil {
			tx.Rollback()
			b.Logger.Error("Transaction rolled back due to error", "error", err)
		} else {
			tx.Commit()
	...
	if len(formatsToRemove) > 0 {
		err := b.formatRepo.RemoveSpecificFormats(ctx, bookID, formatsToRemove)
		if err != nil {
	...
	if len(formatsToAdd) > 0 {
		err = b.formatRepo.AddFormats(tx, ctx, bookID, formatsToAdd) // Adjusted to pass transaction
		if err != nil {
...
func (b *BookUpdaterImpl) updateGenres(ctx context.Context, bookID int, newGenres []string) error {
	...
	if len(genresToRemove) > 0 {
		err := b.genreRepo.RemoveSpecificGenres(ctx, bookID, genresToRemove) // Pass ctx to this method
		if err != nil {
	...
	for _, genre := range genresToAdd {
		genreID, err := b.genreRepo.AddOrGetGenreID(ctx, genre) // Use the updated method with ctx
		if err != nil {
			b.Logger.Error("Error getting genre ID", "error", err)
			return err
		}
		err = b.genreRepo.AddGenre(ctx, bookID, genreID) // Use the refactored AddGenre method
		if err != nil {
