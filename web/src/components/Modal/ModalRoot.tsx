...
// fixed bottom-0 left-0 right-0 z-40 w-full p-4 overflow-y-auto transition-transform bg-white dark:bg-gray-800 transform-none
  return (
    <div className="modal-overlay overflow-hidden fixed inset-0 z-30" data-closing={closing} onClick={handleClose}>
      <div className={`modal-content w-full lg:w-[400px] absolute lg:relative ${closing ? "" : "bottom-0"}`} onClick={(e) => e.stopPropagation()}>
        {children}
      </div>
    </div>
  );
